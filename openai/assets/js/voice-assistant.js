...
export class VoiceAssistant {
    ...
    setupWebSocketHandlers() {
        ...
        this.websocketClient.onConnectionStateChange((state) => {
            this.uiController.showConnectionStatus(state);
            this.uiController.setConnectionState(state === 'connected');
        });
    ...
    setupSpeechHandlers() {
        this.speechManager.onStart(() => {
            ...
            // Notify server
            this.websocketClient.sendMessage({ type: "start_recording" });
        });
    ...
    sendTextMessage() {
        ...
        this.websocketClient.sendMessage({ type: "text", content: text });
    ...
    startKeepalive() {
        this.keepaliveInterval = setInterval(() => {
            if (this.websocketClient.isConnected()) {
                this.websocketClient.sendMessage({ type: "ping" });
            }
        }, 30000); // 30 seconds
    }
