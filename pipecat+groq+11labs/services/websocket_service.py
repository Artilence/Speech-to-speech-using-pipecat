"""
WebSocket manager for handling real-time connections with Pipecat
"""
...
class WebSocketManager:
    """Manages WebSocket connections for pipecat voice chat sessions"""
    ...
    async def send_audio_to_session(self, session_id: str, audio_data: bytes, audio_format: str = "wav"):
        ...
        try:
            ...
            logger.debug(f"🔊 Audio chunk sent to {session_id} ({len(audio_data)} bytes)")
            return True
    ...
    async def send_transcription_to_session(self, session_id: str, text: str, is_final: bool = True):
        """Send transcription result to a specific session"""
        await self.send_to_session(session_id, {
            "type": "transcription",
            "text": text,
            "is_final": is_final,
    ...
    async def notify_processing(self, session_id: str, message: str = "Processing..."):
        """Notify session that processing has started"""
        await self.send_to_session(session_id, {
            "type": "processing",
            "message": message,
            "timestamp": time.time()
    ...
    async def notify_error(self, session_id: str, error_message: str):
        """Send error notification to session"""
        await self.send_to_session(session_id, {
            "type": "error",
    ...
    async def notify_completion(self, session_id: str, result: dict):
        """Send completion notification to session"""
        result.update({
            "type": "completion",
            "timestamp": time.time()
        })
        await self.send_to_session(session_id, result)